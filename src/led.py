...
def __dim_color__(color, threshold=64):
    max_val = max(color)
    if max_val <= 0:
        return 0, 0, 0
...
class Led:
    def __init__(self, pin, led_count, default_color=(0,0,0)):
        ...
        self.default_color = __dim_color__(default_color)

        self.color_cycle_anim = [(0,255,255),(255,255,255),(255,255,0), (255,0,255)]
        self.current_index_anim = 0
        self.default_color_anim = __dim_color__(self.color_cycle_anim[0])
    ...
    def set_led_color(self, index, color, dim_color=True):
        self.__color_semaphore__.acquire()

        if  type(index) == list:
            for i, c in zip(index, color):
                val = __dim_color__(c) if dim_color else c
                self.colors[i] = val
                self.__update_strip__(i)
        else:
            val = __dim_color__(color) if dim_color else color
    ...
    def set_multiplier(self, index, multiplier):
        self.__multiplier_semaphore__.acquire()

        if  type(index) == list:
            for i in index:
                self.ledMultipliers[i] = multiplier
                self.__update_strip__(i)
        else:
            self.ledMultipliers[index] = multiplier
            self.__update_strip__(index)

        self.__multiplier_semaphore__.release()

    def switch_default_color(self):
        self.current_index = (self.current_index + 1) % len(self.color_cycle)
        self.default_color = __dim_color__(self.color_cycle[self.current_index])
